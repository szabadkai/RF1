...
handle = open(raw_input('Enter input file name: '), "rU")

for record in SeqIO.parse(handle, "fasta") :
	try:
		(sequence, strand, max_stop, max_start) = trace_stop_context(record)
		regex = re.compile('(TAG|TAA|TGA)CA[GA][CT]{2}[A]',re.IGNORECASE)
		m = re.match(regex,str(sequence))

		if m:
			if strand == '-1':
				(elso,masodik) = record.seq[:max_start].reverse_complement().translate().split('*',1)
				next = 3*len(elso)
				utr = len(record.seq[:max_start])
			else:
				(elso, masodik) = record.seq[max_stop:].translate().split('*',1)
				next = 3*len(elso)
				utr = len(record.seq[max_stop:])
			print(record.id+'\t'+sequence+'\t'+strand+'\t'+str(next)+'\t3\'utr length: '+ str(utr) )
		else:
			pass
	except:
		pass
